# kiit-inputs — Build & Publish Guide
...
```bash
./gradlew :kiit-inputs:publishAndReleaseToMavenCentral
```
...
step should force loopback pinentry mode before importing:
...
# Clean build outputs
./gradlew :kiit-inputs:clean

# Compile all targets (JVM, Android, iOS)
./gradlew :kiit-inputs:build

# Compile only — no tests
./gradlew :kiit-inputs:assemble
...
./gradlew :kiit-inputs:linkDebugFrameworkIosSimulatorArm64
...
./gradlew :kiit-inputs:jvmTest

# All platform tests
./gradlew :kiit-inputs:allTests

# Publish to Maven Local (~/.m2) for integration testing against other modules
./gradlew :kiit-inputs:publishToMavenLocal
```

Maven Local artifacts are saved to:
```
~/.m2/repository/dev/kiit/kiit-inputs/
```
...
[central.sonatype.com/artifact/dev.kiit/kiit-inputs](https://central.sonatype.com/artifact/dev.kiit/kiit-inputs)
...
```bash
./gradlew :kiit-inputs:signKotlinMultiplatformPublication
```
